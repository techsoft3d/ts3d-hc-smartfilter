...
export class SmartSearchManager {
    ...
    getSmartSearchs() {
        return this._SmartSearchs;

    }

    getSmartSearchByName(name) {
       for (let i=0;i<this._SmartSearchs.length;i++) {
            if (this._SmartSearchs[i].getName() == name)
                return this._SmartSearchs[i];
        }
    }

    getSmartSearchByID(id) {
        for (let i=0;i<this._SmartSearchs.length;i++) {
            if (this._SmartSearchs[i]._id == id)
                return this._SmartSearchs[i];
        }
    }
    getSmartSearchNum() {
        return this._SmartSearchs.length;
    }

    getSmartSearch(pos) {
        return this._SmartSearchs[pos];                
    }

    getSmartSearchID(pos) {
        return this._SmartSearchs[pos]._id;                
    }
   
    removeSmartSearch(id) {
        for (let i=0;i<this._SmartSearchs.length;i++) {
            if (this._SmartSearchs[i]._id == id) {
                return this._SmartSearchs.splice(i, 1);
            }
        }
    }

    updateSmartSearch(pos, SmartSearch) {
        this._SmartSearchs[pos] = SmartSearch;
    }

    updateSmartSearchIsProp(id,isProp) {
        for (let i=0;i<this._SmartSearchs.length;i++) {
            if (this._SmartSearchs[i]._id == id) {
                this._SmartSearchs[i].setProp(isProp);
            }
        }
    }
    ...
    toJSON() {
        let json = [];
        for (let i = 0; i < this._SmartSearchs.length; i++) {
            json.push(this._SmartSearchs[i].toJSON());
        }
        return json;
    }
    ...
    fromJSON(json) {
        ...
        for (let i = 0; i < json.length; i++) {
            let sf = new SmartSearch(this);
            sf.fromJSON(json[i]);
            this.addSmartSearch(sf);
        }
        return json;
    }
    ...
    async evaluateProperties(nodeid)
    {
        let properties = [];
        for (let i = 0; i < this._SmartSearchs.length; i++) {
            if (this._SmartSearchs[i].getProp()) {
                let SmartSearch = this._SmartSearchs[i];

                let stop = false;
                if (this._keepSearchingChildren != undefined ? !this._keepSearchingChildren : !SmartSearch._keepSearchingChildren) {
                    let tnodeid = nodeid;
                    while (1) {
                        ...
                        let res = await SmartSearch.testOneNodeAgainstConditions(tnodeid);
                        if (res) {
                            stop = true;
                            break;
                        }                        
                ...
                let res = await SmartSearch.testOneNodeAgainstConditions(nodeid);
                if (res)
                {
                    let allPropertiesOnNode = await SmartSearch.findAllPropertiesOnNode(nodeid);
                    let text = SmartSearch.getName();
                    if (text == "")
                    {
                        text = SmartSearch.generateString();
                    }
    ...
    getAllProperties(hideIFCProperites = false) {
        ...
        propsnames.unshift("SmartSearch");
